...
class JoelsAwesomeModel(L.LightningModule):
    ...
    def __init__(self) -> None:
        ...
        self.backbone = nn.Sequential(
            ...
            nn.Conv2d(
                1, 32, 3, 1
            ),  # Output: (batch, 32, 26, 26) - 1→32 channels, 28-3+1=26 spatial
            nn.ReLU(),
            # Input: (batch, 32, 26, 26)
            nn.Conv2d(
                32, 64, 3, 1
            ),  # Output: (batch, 64, 24, 24) - 32→64 channels, 26-3+1=24 spatial
            nn.ReLU(),
            # Input: (batch, 64, 24, 24)
            nn.Conv2d(
                64, 128, 3, 1
            ),  # Output: (batch, 128, 22, 22) - 64→128 channels, 24-3+1=22 spatial
            nn.ReLU(),
            # Input: (batch, 128, 22, 22)
            nn.AdaptiveAvgPool2d(
                1
            ),  # Output: (batch, 128, 1, 1) - global average pooling
        )
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Dropout(
                0.5
            ),  # Output: (batch, 128) - randomly zeroes 50% of activations
...
if __name__ == "__main__":
    ...
    trainer = Trainer(
        ...
        callbacks=[PrintCallback(), early_stopping_callback, checkpoint_callback],
    )
