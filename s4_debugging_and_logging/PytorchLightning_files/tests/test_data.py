...
def test_data():
    ...
    assert len(train_images) == N_train, (
        "Train set did not have the correct number of samples"
    )
    assert len(test_images) == N_test, (
        "Test set did not have the correct number of samples"
    )

    assert test_images.shape[1:] == (1, 28, 28), "Input shape incorrect size"

    unique_labels_train = set(train_target.tolist())
    assert all(i in unique_labels_train for i in range(10)), (
        "All labels were not represented in train set"
    )

    unique_labels_test = set(test_target.tolist())
    assert all(i in unique_labels_test for i in range(10)), (
        "All labels were not represented in test set"
    )
