...
def test_model():
    ...
    assert dummy_input.shape == (1, 1, 28, 28), (
        "Input shape does not match - should be [1, 28, 28]"
    )
...
def test_error_on_wrong_shape():
    ...
    with pytest.raises(ValueError, match="Expected input to a 4D tensor"):
        (
            ...
            "ValueError does not recognize incorrect input dimensionality",
        )
    with pytest.raises(
        ValueError, match=r"Expected each sample to have shape \[1, 28, 28\]"
    ):
        (
            model(torch.randn(1, 1, 28, 29)),
            "ValueError does not recognize incorrect input shape",
        )
